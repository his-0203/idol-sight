...
- **V2.48 (2026-06-09)**: 산식 종합 감사 + 처분별 처리 (전 `docs/analysis-formulas-reference.md` 작성 후 비판적 검토). 운영자 요청 "AI 판단으로 우선순위 항목 전부 진행" — **변경 유형을 항목별로 차등**(제 판단): 명백한 편향 수정만 코드, 의도적 설계/미검증 calibration 은 추측 변경 금지([[project_organicity_heuristic_only]] 원칙). **코드 수정**: ① **A1 — 충성도 전환율 시점 매칭**(`loyalty.py`). 기존 `median(56일 peak)/최신구독자` 는 50일 전 방송 peak 를 *오늘* 구독자로 나눠 급성장 채널(데뷔기 MiiWAN)을 **구조적 과소평가**. `subscribers_at(series, at)` 신설(방송 시점 ≤ 최근 스냅샷, 이전이면 최초, 결측 최신 폴백) + `compute_loyalty(..., subs_at=)` 로 **방송별 전환율 → median**. `subs_at` 미제공 시 `median(peaks)/subscribers` 와 동일(하위호환). 표시 `peak_ccv_median`/`subscribers` 불변. `build_fan_loyalty` 가 agg_summary 구독자 시계열 구성해 주입. 기존 편향 테스트(`picks_latest_nonnull_subscribers` 0.015→0.03) 정정 + 신규 4 테스트. **문서/주석 수정(수식 불변)**: ② **A2 — ER 3정의 명시**(organicity 무가중 `(L+C)/V` vs Health/진단 댓글×5 `(L+5C)/V` vs shortsDiagnostic ×100). 통일하면 organicity calibration 깨짐 → 주석으로 구분(`debut_window.py`/`weekly_diagnosis_signals.py`/레퍼런스 §6.2). ③ **B3 — `wow_pct` docstring 정합**(`max(prev,1)`→실제 `/prev`, prev==0→None 이 의도적[0 베이스라인=데이터갭/onset, false-positive 방어]임을 명시). ④ **C — `classify_direction` deprecation 주석**(실경로는 `_compare_direction`, 테스트 있어 미삭제) + `debutWindowBuckets.ts` stale "15일/9 named" 주석 정정(20일·7 named+Pre/Post). ⑤ **B2 — growth 카피**: 감사로 `GrowthTrajectoryPanel` 이 `change_4w`(28일) 아닌 `fmtPrevRecent`(prev7d→recent7d)를 렌더함을 발견 → 단어·숫자 **이미 동일 지평**(비-이슈), 힌트 1줄만 명확화. **백로그 이관(코드 미변경, 운영자 판단/데이터 검증 게이트)**: **B1**(Health risk_factor 가 Reach 압축+Intimacy 이중 — 의도적 설계 `test_controversy_compresses_all_four_factors` 인코딩, 추측 번복 안 함), **B4**(growth reach 조회수 fallback "상승" 편향), **B5**(정규화 log 가 news 에만, subs/views heavy-tail), **B6**(negative_ratio 200/run 샘플링 편향) — 전부 전 그룹 점수 이동이라 실측 분포 검증 후. 마이너 잔여(충성도 임계 3종 분산/balance_basis 50K 절벽/reactivity before=0 floor/news 날짜게이트 비대칭/SOV 모멘텀 dead 가중)도 백로그. 상세 `[[project-followup-backlog]]` "산식 종합 감사" 섹션. worker 724(+4) + frontend 184 통과, tsc clean. 병렬 추출/수정 에이전트 + 핵심 상수 직접 spot-check. A1 외 전부 주석/문서/카피라 migration·배포 gate 무관(A1 은 다음 aggregate cron 이 재집계).

- **V2.48.1 (2026-06-11)**: cron 워크플로 **타이밍 하드닝 2건** (운영 장애 사후 분석, 워크플로 전용 — 코드·migration·테스트 무관). ① **analyze-weekly 06-10(수) cancelled 의 근본 원인은 시간대가 아니라 Gemini 선불 크레딧 소진** — 429 RESOURCE_EXHAUSTED retry 폭풍(백오프 sleep 최대 ~140s, 콜당 10회 예산)이 `timeout-minutes: 30` 을 소진 → job cancelled. `if: failure()` 는 cancelled 에 미발화라 **Discord 무알림**, DB 미기록(ws=06-07 interim 6카드는 06-08 수동 dispatch 산출, 06-10 데이터 미반영 상태). **크레딧 충전은 운영자 액션**(AI Studio). 워크플로 하드닝: cron 을 요일별 2엔트리(`0 14 * * 0` final / `0 14 * * 3` interim)로 분리하고 bounds 가 `github.event.schedule` 로 kind 판정 + final week_end 를 '어제'→'**가장 최근 토요일**' 앵커로 — GitHub tick 지연(실측 3~4h)이 UTC 자정을 넘겨 요일이 바뀌어도(일→월) final 이 interim 으로 오분류되거나 주가 밀리지 않음(blank dispatch 는 기존 요일 휴리스틱 유지). notify-fail `if: failure() || cancelled()` 로 timeout 도 알림. 운영자 요청 '새벽 3시 이동'은 **미적용** — 원인이 시간대가 아니고, 일요일을 KST 새벽 3시(=UTC 18:00 일요일)로 옮기면 UTC 자정 버퍼가 10h→6h 로 줄어 지연 오분류 리스크가 오히려 증가. ② **collect-ccv OWIS 06-10 미수집의 근본 원인은 GitHub tick 지연** — 단일 트리거(18:45 KST)가 실측 +2:40~+3:00 지연돼 윈도우가 21:25+ KST 에 시작, 그 전에 끝난 OWIS 이른 저녁 방송과 무겹침(코드 정상 — 같은 윈도우에서 skinz 방송 꼬리 4샘플·wegosix 22시 방송 21샘플은 정상 적재, OWIS video ID 도 매 샘플 조회됐으나 live 아님). 06-09 는 러너가 루프 3.5h 차 사망(V2.38.1 '관찰(미수정)' 리스크의 실현, notify 스텝도 못 돔). 트리거 2개(`45 6`/`45 9` UTC = KST 15:45/18:45)로 보강, 기존 concurrency group(`cancel-in-progress: false`)이 직렬화 — 정시면 15:45~00:45 연속, 둘 다 ~3h 지연이면 18:45~03:45 연속, 1차 중간 크래시 시 2차가 저녁 윈도우 백필. 중복 샘플은 UPSERT 멱등 + 쿼터 ~1유닛/샘플이라 무해. **잔여 리스크**(수용): 1차 tick 완전 드롭 + 2차 max 지연 동시 발생 시 이른 저녁 갭 — 지연은 시간대 부하 상관이라 혼합 극단은 드묾. bounds 산식은 요일·지연 시나리오 7종 로컬 전수 검증.

- **V2.49 (2026-06-11)**: Debut Window **롤링 윈도우** — 데뷔일 고정 ±60일 스냅샷에서 MiiWAN 나이를 따라 20일에 한 칸씩 굴러가는 7버킷(140일) 창으로 전환 (운영자 요청 — 데뷔 후 타임라인 확장, 과거 버킷 퇴장, D-Day 자연 퇴장). worker `bucket_for`: `Post` catch-all 폐기 → `d≥10 → D+20k (k=(d-10)//20+1)` 산술 무한 생성 총함수화 (`Pre` 는 유지 — 창이 과거로 안 밀림, `WINDOW_BUCKETS` 는 고정 5항목으로 축소), migration 0085 가 기존 Post 행 in-place 재배치 + summary DELETE (0073 패턴). 표시 창은 **서버 계산** — `functions/lib/debutWindowBuckets.ts` 전면 재작성 (`FRONTEND_BUCKET_MAP` identity 맵 폐기, `displayBuckets(ageDays)` = 오늘 버킷을 오른쪽 끝(최소 D+60)으로 한 연속 7개 + `currentBucket`/`isValidBucketLabel`/`debutAgeDaysKST`): summary API 가 D1 의 MiiWAN debut_date(KST 경과일)로 계산해 `window.buckets`/`current_bucket` 메타 응답 (CASE 빌더 제거 — V2.34 부터 1:1 identity 라 passthrough), 3 컴포넌트 (DebutWindowKPI/CompetitorOrganicityBar/DebutWindowVideoTable) 가 정적 `DISPLAY_BUCKETS` 대신 메타 렌더 (`picked ?? defaultBucket` 패턴, fallback = 데뷔 전 고정 창 `DEFAULT_DISPLAY_BUCKETS`, VideoTable 은 mount 시 summary 1회 추가 호출). 기본 선택 탭 = `current_bucket` (오늘 버킷). 슬라이드 기준은 MiiWAN 나이 **전 그룹 공통** (각 그룹 라벨은 자기 데뷔 기준 D±N → 동일 라이프스테이지 코호트 비교 유지, 운영자 확정). wegosix (debut 08-31 placeholder) 는 미도래 버킷 빈 칸, BTHD 는 Undated 무변경 (운영자 확정). **D+70 (2026-08-25) 전까지 화면 완전 동일** (창=종전 D-60~D+60, current=D-Day). weekly_diagnosis 는 published_at 범위 조회라 무영향. cross-language 경계 동일성은 `debutWindowBuckets.test.ts` BOUNDARY_FIXTURE ↔ worker parametrize 양쪽 핀. 스펙/플랜 `docs/superpowers/{specs,plans}/2026-06-11-debut-window-rolling-window*`. worker 727 + frontend 214 통과, tsc clean. **migration 0085 운영자 원격 apply 필요** (`gh workflow run migrate.yml`) — 적용 전에도 표시 창엔 Post 가 원래 없어 graceful, 적용 후 다음 organicity cron 이 summary 재집계 (D+80+ 버킷은 worker cron 재채점 후 노출).

- **V2.50 (2026-06-16)**: 라이브 채팅 **종료-후 긍/부정 분류 리포트** (MiiWAN). 라이브가 끝난 방송의 YouTube 채팅 **리플레이를 비공식 `get_live_chat_replay` 스크레이핑**(Data API 쿼터 0, DC/theqoo/RSS 비공식 수집 철학과 일치)으로 긁어 방송 1건당 대표 긍/부정 멘트 + 비율 추정 리포트를 만든다. 종료 감지는 별도 장치 없이 **`live_ccv_samples`(V2.38 이 밤새 기록한 라이브 신호)를 소비** — `videos.list(actualEndTime)` 로 종료·`min_age_min`(기본 30분, 리플레이 준비 여유) 경과분만 선별. migration 0090: `live_chat_messages`(video_id+msg_id PK 멱등 raw, 재분석 원천) + `live_chat_reports`(video_id PK = 처리 완료 마커 → 멱등·재시도 제어). `collectors/live_chat.py` `LiveChatReplayScraper`(watch 페이지 INNERTUBE_API_KEY/clientVersion/continuation 추출 → youtubei 페이지네이션, `MAX_MESSAGES`/`MAX_PAGES` 가드, 리플레이 없으면 `[]` graceful) + `ended_broadcasts()`. `analysis/live_chat_report.py` `build_report`: dedup·도배·단문 제거 후 타임라인 stride 로 `SAMPLE=500` 표본 → **Gemini 1회** structured output(positive/negative_ratio + 대표 quote 각 3~5 + themes + summary, `sentiment.py` _Gemini DI 패턴 재사용). `cli.py collect-live-chat`(`--group miiwan`, `collect-ccv` 와 대칭, 후보=최근 3일 윈도∖기존 리포트, 전-후보 실패 시에만 sentinel). `collect-live-chat.yml` cron 2엔트리 — **KST 04:00(정상, 그날 밤 종료분 그 새벽 수집) / 12:00(안전망, 리플레이 미준비분 재시도)**; 3일 윈도는 재시도 상한일 뿐 정상 latency 는 "방송 끝→그날 새벽 수 시간". `/api/miiwan-live-chat`(최근 10건, report_json 파싱, 테이블 없으면 빈 배열 graceful) + MiiWANBriefing **"라이브 채팅 반응"** 섹션(방송별 카드 — 긍/부정 비율 바[추정 라벨] + 대표 멘트 + 테마 칩, 이벤트 타임라인 아래). 비율은 **표본 기반 추정치**(전수 아님). 비범위(후속): 치지직, 전수 분류·시간대 추이, ccv_tracked 전체 확대, 슈퍼챗. 스펙/플랜 `docs/superpowers/{specs,plans}/2026-06-16-live-chat-sentiment*`. worker 754(+17) + frontend 272 통과, ruff/tsc clean. **migration 0090 운영자 원격 apply 필요**(`gh workflow run migrate.yml`) — 적용 전엔 `/api/miiwan-live-chat` 가 빈 배열 graceful, 적용 후 다음 collect-live-chat cron 이 리포트 적재.

**다음 단계 (우선순위)**: S급 (RBAC + Cloudflare Access, 감사 로그, 본체 마스킹, **티켓 매진속도 collector** — 다음 데뷔-크리티컬, 별도 brainstorm 필요). **알려진 잔여**: bdawn DC 갤러리 노이즈(갤러리 ID 도메인 재검토), twitter 수집 비활성, 산식 감사 B1/B4/B5/B6 calibration(백로그), **Gemini 크레딧 충전 후 06-07 주 interim 재생성**(V2.48.1). 라이브 CCV 는 V2.38 로 완료. 자세한 우선순위는 대화 컨텍스트 또는 v2-roadmap.md 참고.
