...
1. 그룹의 `live_ccv_samples`에서 최근 `WINDOW_DAYS=28`일(`sampled_at >= now-28d`) 행을 fetch.
...
- `worker/tests/unit/test_loyalty.py` 신규: median/전환율/구간보간 경계값/가드(0방송·1방송·subscribers 결측)/basis 라벨.
...
- collect-ccv cron이 다음 윈도우부터 8개 그룹 수집 → 28일 축적되며 점수 안정화.
