...
CHALLENGE_DISCOVERY_PROMPT = (
    ...
    "- **이름(name)에는 반드시 '가수(그룹)명 - 곡명'을 포함**하라 (검색에 쓰임). "
    "예: 'tripleS - Baby Flower 챌린지'. 곡명만 단독으로 두지 말 것.\n"
    ...
    "뮤직비디오·티저가 아니라, 그 챌린지(안무·포맷)를 따라 한 짧은 세로 클립의 URL. "
    "**가능하면 첫 번째는 원곡 아티스트(그룹) 본인이 찍은 챌린지 클립**을 넣고, 그 다음 "
    "다른 클립. 반드시 검색에서 실제로 확인한 것만, 없으면 비워둘 것 (지어내지 말 것).\n\n"
...
CHALLENGE_STRUCTURE_SYSTEM = (
    ...
    "뮤직비디오 제외). **원곡 아티스트 본인 클립을 맨 앞**에 두고 그 다음 다른 클립. "
    "없으면 빈 배열.\n"
...
CHALLENGE_CLASSIFY_SYSTEM = (
    ...
    "없는 챌린지를 지어내지 말 것.\n"
    "- 근거 있는 한 **최대 10개까지** 식별하라. 조회수 1·2위 메가히트뿐 아니라 "
    "**떠오르는 중소·신생 그룹의 챌린지**도 포함하라 — 조회수 절대값이 낮아도 "
    "context.videos 에 있으면 후보다(롱테일).\n"
    "- 같은 곡/챌린지의 여러 영상을 하나로 묶어라. 조회수·영상 수가 많을수록 상위.\n"
    "- name: 반드시 '가수(그룹)명 - 곡명' 포함 (예: 'MEOVV - Hands Up 챌린지').\n"
    "- description: **영상 목록에서 관찰 가능한 내용(안무 포인트·밈의 웃음 포인트·"
    "따라 하는 방식)만** 한두 줄로 서술하라. **소속사·레이블·데뷔 시기·멤버 구성 등 "
    "목록에 없는 외부 사실은 쓰지 말 것** — 확인 불가이며 환각 위험(예: 소속사 오기). "
    "가수·곡명 정도는 name/title 에서 확인되는 선까지만.\n"
    ...
    "momentum(rising/peaking/declining/unknown), confidence, "
