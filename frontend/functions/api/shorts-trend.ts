...
export const onRequestGet: PagesFunction<{ DB: D1Database }> = async ({ env }) => {
  // Latest stats row per video via a single MAX-snapshot JOIN — was three
  // correlated per-row subqueries (views/likes/comments), i.e. 3× the index
  // seeks over 400+ rows. PK(video_id, snapshot_at) makes the MAX an O(log n)
  // seek. (Same pattern as videos-all.ts / group/[key].ts.)
  const latestJoin = `
  ...
  // All independent — issue concurrently instead of ~6 serial round-trips.
  // challenges is wrapped so a missing table (pre-remote-migration) still
  // degrades to [] without failing the whole request.
  const [groups, trendRows, miiwanShorts, summaryNow, members, challenges] =
    await Promise.all([
      d1Query<GroupRow>(env.DB,
        `SELECT key, name, name_kr, context_keywords
           FROM groups WHERE is_active = 1`),
      // 경쟁사(=MiiWAN 제외) 숏폼, 최근 90일.
      d1Query<TrendRow>(env.DB,
      ...
      // MiiWAN 숏폼 (진단용 표본 — 90일 제한 없음, 단 무제한 방지 위해 상한).
      d1Query<ShortRow>(env.DB,
        `SELECT v.video_id, v.title, v.published_at, v.viral_velocity_ratio,
                s.views AS views, s.likes AS likes, s.comments AS comments
           FROM youtube_videos v
           ${latestJoin}
          WHERE v.is_short = 1 AND v.group_key = ?
          ORDER BY v.published_at DESC
          LIMIT ?`,
        [SELF_KEY, TREND_LIMIT]),
      d1Query<SummaryRow>(env.DB,
        `SELECT group_key, yt_subscribers
           FROM agg_summary
          WHERE group_key = ?
          ORDER BY snapshot_at DESC LIMIT 1`, [SELF_KEY]),
      d1Query<{ composite_score: number | null }>(env.DB,
